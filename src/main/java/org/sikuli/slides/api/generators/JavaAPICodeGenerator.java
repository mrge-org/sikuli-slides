...
public class JavaAPICodeGenerator implements CodeGenerator{
    ...
    public Step generateStep(TargetAction targetAction, int number){
        ...
        // In SikuliX, TargetAction holds a Pattern directly
        Pattern pattern = targetAction.getPattern();
        if (pattern == null) {
            return null;
        }
        // Try to extract the underlying Image and BufferedImage for file output
        BufferedImage targetImage = null;
        Image img = pattern.getImage();
        if (img != null) {
            try {
                targetImage = img.get(); // may return BufferedImage
            } catch (Throwable t) {
