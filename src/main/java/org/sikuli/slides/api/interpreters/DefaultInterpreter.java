...
public class DefaultInterpreter implements Interpreter {
    ...
    static class WaitActionInterpreter extends RegexActionInterpreter {
        ...
        protected Action interpret(Slide slide, SlideElement element, String[] arguments){
            ...
            Pattern pattern = (new ContextImageTargetInterpreter()).interpret(slide);
            if (pattern == null)
                return null;
    ...
    static class ScreenLocationInterpreter implements SpatialRelationshipInterpreter{
        ...
        public SpatialRelationship interpret(Slide slide, SlideElement element) {            
            ...
            return new SpatialRelationship(){
                @Override
                public Region apply(Context input) {
                    Region screenRegion = input.getScreenRegion();
                    int x = screenRegion.getX() + (int) Math.round(xmin * screenRegion.getW());
                    int y = screenRegion.getY() + (int) Math.round(ymin * screenRegion.getH());
                    int w = (int) Math.round((xmax - xmin) * screenRegion.getW());
                    int h = (int) Math.round((ymax - ymin) * screenRegion.getH());
    ...
    static class TargetLocationInterpreter implements SpatialRelationshipInterpreter{
        ...
        public SpatialRelationship interpret(Slide slide, SlideElement element) {        
            ...
            final Pattern pattern = (new ContextImageTargetInterpreter()).interpret(slide, targetElement);
            if (pattern == null)
                return null;
            ...
            return new SpatialRelationship(){
                @Override
                public Region apply(Context context) {
                    try {
                        Region found = context.getScreenRegion().find(pattern);
                        if (found != null) {
                            return new Region(found.getX() + x, found.getY() + y, width, height);
                        }
                    } catch (FindFailed e) {
